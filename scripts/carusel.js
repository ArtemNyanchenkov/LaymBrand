...
Ant.prototype.dotOff = function(num) {
    this.indicatorDotsAll[num].style.cssText = 'background-color:#464646; cursor:default;'
};
...
Ant.initialize = function(that) {
    ...
    if(that.elemCount <= that.options.elemVisible) {   // Отключить навигацию
        that.options.auto = true; that.options.touch = true;
        that.options.arrows = true; that.options.dots = true;
        that.leftArrow.style.display = 'block'; that.rightArrow.style.display = 'block'
    };

    if(!that.options.loop) {       // если нет цикла - уточнить количество точек
        that.dotsVisible = that.elemCount - that.options.elemVisible + 1;
        that.leftArrow.style.display = 'block';  // отключить левую стрелку
        that.touchPrev = true;    // отключить прокрутку прикосновением вправо
    ...
    if(that.options.arrows) {  // инициализация стрелок
        ...
        that.leftArrow.style.display = 'block';
        that.rightArrow.style.display = 'block'
    };
